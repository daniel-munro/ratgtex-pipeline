# Source: https://github.com/broadinstitute/gtex-pipeline/blob/master/qtl/leafcutter/src/cluster_prepare_fastqtl.py
...
if __name__=='__main__':
    ...
    with tempfile.NamedTemporaryFile(dir=args.output_dir) as tmp:
        ...
        subprocess.check_call(
            ...
            ## LeafCutter recommends regtools now: -DM
            'python3 '+os.path.join(args.leafcutter_dir, 'leafcutter_cluster_regtools_py3.py' \
                +' --juncfiles '+tmp.name \
    ...
    subprocess.check_call('gzip {}_pooled'.format(os.path.join(args.output_dir, args.prefix)), shell=True)
    subprocess.check_call('gzip {}_refined'.format(os.path.join(args.output_dir, args.prefix)), shell=True)
    ...
    pc_df.index.name = 'ID'
    pc_df.to_csv(os.path.join(args.output_dir, args.prefix+'.leafcutter.PCs.txt'), sep='\t')
